function [orbits] = Input_Orbit(folder,reference_orbits)    
    ...
    for Isc = 1:1:str2double(reference_orbits) 
        ...
        KEP_RV_FILE(Isc) = ["RV"];
            ...
            theta(Isc) = ["0.0"]; % deg
            % RV orbit
            RV_x(Isc,1) = ["5869.240124394930"];
            RV_x(Isc,2) = ["-2389.971076791800"];
            RV_x(Isc,3) = ["-2471.324786352320"];
            RV_v(Isc,1) = ["3.83879147867145"];
            RV_v(Isc,2) = ["3.94206409833092"];
            RV_v(Isc,3) = ["5.32186352529300"];
            % File orbit
