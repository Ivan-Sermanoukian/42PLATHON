      ...
      if (OutFlag) {
         ...
         for(Isc=0;Isc<Nsc;Isc++) {
            if (SC[Isc].Exists) {
               ...
               for(i=0;i<SC[Isc].Nw;i++) fprintf(Hwhlfile[Isc],"%lf ",SC[Isc].Whl[i].H);
               fprintf(Hwhlfile[Isc],"\n");
               if (SC[Isc].Nmtb > 0) {
                  for(i=0;i<SC[Isc].Nmtb;i++) fprintf(MTBfile[Isc],"%lf ",SC[Isc].MTB[i].M);
                  fprintf(MTBfile[Isc],"\n");
               }
